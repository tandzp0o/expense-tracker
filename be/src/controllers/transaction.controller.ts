...
export const createTransaction = async (req: any, res: Response) => {
  ...
  try {
    ...
    // Cập nhật số dư ví
    wallet.balance = type === TransactionType.INCOME 
...
export const getTransactions = async (req: any, res: Response) => {
  try {
    ...
    const { startDate, endDate, type, category, walletId, note } = req.query;
    const userId = req.user.uid;
...
export const updateTransaction = async (req: any, res: Response) => {
  ...
  try {
    ...
    // Kiểm tra ví cũ
    const oldWallet = await Wallet.findById(currentTransaction.walletId).session(session);
    if (!oldWallet) {
      await session.abortTransaction();
      return res.status(404).json({
        success: false,
    ...
    if (walletId && walletId !== currentTransaction.walletId.toString()) {
      const foundWallet = await Wallet.findOne({ _id: walletId, userId }).session(session);
      if (!foundWallet) {
        await session.abortTransaction();
        return res.status(404).json({
          success: false,
    ...
    if (currentTransaction.type === TransactionType.INCOME) {
      oldWallet.balance = Number(oldWallet.balance) - Number(currentTransaction.amount);
    } else {
      oldWallet.balance = Number(oldWallet.balance) + Number(currentTransaction.amount);
    }
