...
export const Eyebrow: React.FC<{
  ...
  <span
    className={cn(
      "text-[11px] font-semibold uppercase tracking-[0.09em] text-ledger-muted",
      className,
    )}
  >
    {children}
...
const buttonClasses = (
  ...
  cn(
    ...
    variant === "danger" && "bg-ledger-out text-white hover:brightness-110",
    block && "w-full",
  );
...
export const PageHeader: React.FC<{
  ...
  <header className="flex flex-col gap-4 border-b border-ledger-line pb-5 pt-6 sm:flex-row sm:items-end sm:justify-between lg:pt-9">
    <div className="min-w-0">
      <h1 className="text-[24px] font-semibold leading-tight tracking-[-0.02em] text-ledger-ink lg:text-[26px]">
        {title}
      </h1>
      {subtitle ? (
        <p className="mt-1 text-[13.5px] text-ledger-ink-2">{subtitle}</p>
      ) : null}
    </div>
    {actions ? (
...
/**
 * The one big number of a page, with its supporting figures in a hairline-
 * separated strip beside it rather than in a row of equal cards.
 */
export const HeroStrip: React.FC<{
  ...
  stats?: HeroStat[];
  children?: React.ReactNode;
}> = ({ label, value, caption, stats = [], children }) => (
  <section className="border-b border-ledger-line py-6 lg:py-8">
    <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
    <div className="min-w-0">
      <p className="text-[13.5px] text-ledger-ink-2">{label}</p>
      <div className="mt-1.5 text-[38px] font-semibold leading-none tracking-[-0.03em] text-ledger-ink sm:text-[44px] lg:text-[52px]">
        {value}
      </div>
      {caption ? (
        <p className="mt-3 text-[13.5px] text-ledger-ink-2">{caption}</p>
      ) : null}
    </div>
    {stats.length ? (
      <dl className="grid grid-cols-3 divide-x divide-ledger-line lg:flex lg:shrink-0">
        {stats.map((stat) => (
          <div
            className="min-w-0 px-2.5 first:pl-0 last:pr-0 sm:px-4 lg:px-6"
            key={stat.label}
          >
            <dt className="truncate">
              <Eyebrow>{stat.label}</Eyebrow>
            </dt>
            {/* Three full VND figures have to fit a 390px screen side by side,
                so the phone size is set by "+15.000.000 ₫", not by taste. */}
            <dd className="mt-1.5 text-[12.5px] font-semibold min-[400px]:text-[13.5px] sm:text-[16px] lg:text-[19px]">
              {stat.value}
            </dd>
          </div>
        ))}
    ...
    {/* Extra content (a progress track, say) spans the whole strip rather
        than only the width of the big number. */}
...
export const Section: React.FC<{
  ...
  className?: string;
  bare?: boolean;
}> = ({ title, meta, action, children, className, bare }) => (
  <section className={cn(!bare && "border-b border-ledger-line", "py-6", className)}>
    <div className="mb-3 flex items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        {meta ? (
          <span className="shrink-0 rounded-full bg-ledger-canvas px-2 py-0.5 text-[12px] text-ledger-ink-2">
            {meta}
          </span>
        ) : null}
      </div>
...
/**
 * A 6px progress track. Past 100% it fills and turns rose instead of being
 * clamped quietly, so an overspent budget reads as one at a glance.
 */
export const Track: React.FC<{
  percent: number;
  tone?: "spend" | "accent" | "in";
  className?: string;
}> = ({ percent, tone = "spend", className }) => {
  ...
  return (
    <div
      ...
      className={cn("h-1.5 w-full overflow-hidden rounded-full bg-ledger-canvas", className)}
      role="progressbar"
    >
      <div
        className={cn(
